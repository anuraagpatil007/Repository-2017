...
if __name__ == "__main__":
   ...
   for line in sys.stdin:
       #Split each line based on paperid,author names and title words 
       lineInput=line.split(':::')
       #check if title word's  part is having value or not 
       if lineInput[2] is not None:		
 	   titleList=lineInput[2].split(' ')
       #check if author name's part is having value or not
       if lineInput[1] is not None:
	    authors=lineInput[1].split('::')
       #traverse through the title word list
       for title in titleList:
		#strip leading and trailing white spaces
		word = title.strip()
		#remove punctuations so that words like "program" and "program." will be treated to be same
		word = word.translate(None, string.punctuation)
		#check for excluding stopwords and words with length less than 2
		if word not in allStopWords and len(word)> 2: 
			#traverse through the author name's list
			for author in authors:
			    #print the author name and the title word to the stdout for furter processing by reducer.
